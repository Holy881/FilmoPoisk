...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const POSTER_SIZE_CARD = 'w342'; // Используется для плиток на полке
    ...
    function renderShelf(shelfElement, moviesData) {
        ...
        setTimeout(() => updateShelfControls(shelfElement), 50);
    }
    ...
    function updateShelfControls(shelfElement) {
        ...
        if (!grid || !prevArrow || !nextArrow || !gridWrapper) return;
    ...
    function initShelves() {
        document.querySelectorAll('.movie-shelf').forEach(shelfElement => {
            ...
            if (!grid || !prevArrow || !nextArrow || !gridWrapper) {
            ...
            setTimeout(() => updateShelfControls(shelfElement), 100); 
        });
    }
